/**
 * Builds a Ruleset from DOC_CLASSES and ABILITY_EFFECT_MAPPINGS.
 * Used at module load by defaultRuleset to produce the default ruleset.
 */

import type { EffectType } from "../AbilityData";
import { ABILITY_EFFECT_MAPPINGS } from "../parsed/AbilityEffectMappings";
import type { DocClass, DocAbility } from "../parsed/SkillTreeContent";
import type {
  Ruleset,
  RulesetClass,
  RulesetArchetype,
...
import { getDefaultEffectParams, getTargetingParams } from "./defaultEffectParams";
...
function buildAbilitiesAndSlots(
  ...
  for (const docClass of docClasses) {
    for (const arch of docClass.archetypes) {
      ...
      for (const docAbility of arch.abilities) {
        ...
        const cost: RulesetAbilityCost = {
          ...defaultCost(docAbility.type, docAbility.tier),
          ...(mapping?.costOverrides ?? {}),
        };

        const trigger = mapping?.trigger
          ? {
              type: mapping.trigger.type,
              triggeredEffect: {
                type: mapping.trigger.triggeredEffect.type,
                params: { ...mapping.trigger.triggeredEffect.params },
              },
              ...(mapping.trigger.condition && { condition: mapping.trigger.condition }),
            }
          : undefined;
        ...
        const def: RulesetAbilityDef = {
          ...
          trigger,
          ...(mapping?.returnToStoredPositionAfterExecute && { returnToStoredPositionAfterExecute: true }),
        };
        abilities.push(def);
...
export function buildRulesetFromDoc(docClasses: DocClass[]): Ruleset {

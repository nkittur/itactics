...
export function generateRecruitSkills(
  ...
  const tiers: (1 | 2 | 3)[] = [1, 1, 2, 3]; // T1=tier1, T2=tier1, T3=tier2, T4=tier3
  const abilities: GeneratedAbility[] = [];

  for (let i = 0; i < 4; i++) {
    const slot = theme.progression[i]!;
    const tier = tiers[i]!;
    const ability = generateAbility(slot, tier, theme.id, theme.weaponAffinity, rng);

...
class TownWindow(qtw.QWidget):
   ...
   def initHiringPoolTable(self):
      ...
      self.hiringPoolTable.setColumnCount(len(self.statTableList))
      self.hiringPoolTable.setSelectionBehavior(QTableWidget.SelectRows);

      self.hiringPoolTable.setHorizontalHeaderLabels(self.statTableList)

      for x in range(numPlayers):
         player = Player()

         self.parent.hiringPool[player.id] = player
   ...
   def onHire(self):
      ...
      player = self.parent.hiringPool.get(int(playerId))
      if player:
         print('Hired ' + player.name)

         self.parent.party[int(playerId)] = player

         x = len(self.parent.party)-1
         self.partyTable.setRowCount(len(self.parent.party))
